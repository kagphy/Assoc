...
class AppController extends Controller {
	...
	public function changeExercice($exercice) {
		$this->Session->write('Dossier.Exercice', $exercice);
		$this->redirect($this->referer());
	}

	public function beforeFilter() {
		$this->Session->write('Exercice', 2014);
		/**
		...
		if (isset($_SESSION['Dossier']['Eleve']['EleveId'])) {
			...
			$personne = $_SESSION['Dossier']['Eleve']['EleveId'];
			$this->loadModel('Fourniture');
			$bon = $this->Fourniture->query("SELECT * FROM bal_vue_e300_bon WHERE ConseilFCPEId=$conseilfcpe AND Exercice=$exercice AND PersonneId=$personne;");
			for ($i=0; $i < sizeof($bon); $i++) {
				$bonoperation[$i][$bon[$i]['bal_vue_e300_bon']['BonOperationId']] = $bon[$i]['bal_vue_e300_bon']['TypeDeBonNom'];
			}
			$this->set('bonoperation', $bonoperation);
		}
